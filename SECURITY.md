...
- The TOTP seed for the `/admin` panel login:
  `~/.secrets/talus/totp_secret` (0600); deployed as a Workers secret.
  Generated by `scripts/setup-totp.sh`, which displays it exactly once.
  Rotate with `scripts/setup-totp.sh --rotate` (invalidates all previous
  codes).
