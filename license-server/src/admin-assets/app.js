// ── Talus Admin Panel — frontend (worker-hosted) ──────────────────────────
// Auth via HttpOnly session cookie (set by /admin/login). All API calls use
// credentials: 'include' — no token is ever stored in or read by this page.
...
async function api(path, opts = {}) {
  ...
  if (res.status === 401) { location.replace('/admin'); return { ok: false, status: res.status, data }; }
...
async function boot() {
  const { ok } = await api('/admin/session-check');
  if (!ok) { location.replace('/admin'); return; }
  $('session-label').textContent = 'session active';
  refresh();
