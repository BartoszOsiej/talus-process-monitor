...
/// NOT pretty: they resolve to the JWT through the license server instead.
pub fn is_pretty(key: &str) -> bool {
    let k: String = key.trim().chars().filter(|c| !c.is_whitespace()).collect();
    let upper = k.to_uppercase();
    upper.starts_with(&format!("{PREFIX}-")) && upper.matches('-').count() >= 6
}

/// Generate a fresh SHORT license key: TALUS-XXXXX-XXXXX-XXXXX-XXXXX.
/// 104 bits from OsRng → 20 Crockford base32 chars (no I/L/O/U). The signed
/// JWT is bound to this key server-side and fetched at activation time.
pub fn new_short_key() -> String {
    use rand::rngs::OsRng;
    use rand::RngCore;
    let mut bytes = [0u8; 13]; // 104 bits of entropy → 20 base32 chars
    OsRng.fill_bytes(&mut bytes);
    let body = &b32_encode(&bytes)[..20];
...
#[allow(dead_code)] // used by tests and future verify-path tooling
...
mod tests {
    ...
    const SAMPLE: &str = "eyJhIjoxfQ.MEUCIQDX";
    const WEIRD: &str = "TALUS-1A2B3-TEST";
    ...
    fn checksum_catches_typos() {
        ...
        // Flip one character in the middle group.
